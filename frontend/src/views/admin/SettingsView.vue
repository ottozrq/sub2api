              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    >
                      {{
                        (form.payment_product_name_prefix || "熔鉴AI") +
                        " 100 " +
...
function hydrateLoginAgreementDocumentTemplate(doc: LoginAgreementDocument): LoginAgreementDocument {
  if (doc.id === "terms" && !doc.content_md?.trim()) {
    const termsTemplate = defaultLoginAgreementDocuments()[0];
    return {
      ...doc,
      title: doc.title?.trim() || termsTemplate.title,
      content_md: termsTemplate.content_md,
    };
  }
  if (doc.id === "privacy" && !doc.content_md?.trim()) {
    const privacyTemplate = defaultLoginAgreementDocuments().find(
      (item) => item.id === "privacy",
    );
    if (privacyTemplate) {
      return {
        ...doc,
        title: doc.title?.trim() || privacyTemplate.title,
        content_md: privacyTemplate.content_md,
      };
    }
  }
  return doc;
    ...
    form.login_agreement_documents =
      ...
      settings.login_agreement_documents.length > 0
        ? settings.login_agreement_documents.map((doc) => ({
          ...
          })).map(hydrateLoginAgreementDocumentTemplate)
