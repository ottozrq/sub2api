...
export function resolveDocumentTitle(routeTitle: unknown, siteName?: string, titleKey?: string): string {
  const requestedSiteName = typeof siteName === 'string' ? siteName.trim() : ''
  const normalizedSiteName = requestedSiteName && !/^sub2api$/i.test(requestedSiteName) ? requestedSiteName : '熔鉴AI'
